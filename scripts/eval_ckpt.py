...
def main():
    ...
    plot_substructure(
        ...
        tau32_gen=tau32,
        d2_gen=d2,
        tau21_sim=tau21_jetclass,
        tau32_sim=tau32_jetclass,
        d2_sim=d2_jetclass,
        save_fig=True,
    ...
    plot_full_substructure(
        data_substructure_gen=data_substructure,
        data_substructure_sim=data_substructure_jetclass,
        keys=keys,
        save_fig=True,
        save_folder=img_path,
    ...
    for jet_type, jet_type_idx in jet_types_dict.items():
        ...
        if (
            ...
            or "part_charge" not in part_names_sim
        ...
        # fmt: off
        is_charged_sim = np.abs(data_sim_this_type[:, :, idx_part("part_charge")]) == 1
        is_charged_gen = np.abs(data_gen_this_type[:, :, idx_part("part_charge")]) == 1
